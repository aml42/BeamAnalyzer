...
def test_beam_analyzer_12m():
    ...
    analyzer = BeamAnalyzer(support_positions, loads, inertia)
    results = analyzer.analyze()
    assert results['max_deflection_per_span'][0]['max_deflection'] == pytest.approx(-1.4e-3, abs=0.15e-3)
    assert results['max_deflection_per_span'][1]['max_deflection'] == pytest.approx(-0.42e-3, abs=0.05e-3)
    assert results['max_deflection_per_span'][2]['max_deflection'] == pytest.approx(-0.42e-3, abs=0.05e-3)
    assert results['max_deflection_per_span'][3]['max_deflection'] == pytest.approx(-1.4e-3, abs=0.15e-3)
